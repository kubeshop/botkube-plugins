...
namespaces:  include:    - "dev"    - "prod"  exclude: [] ### Labels[​](#labels"DirectlinktoLabels")
...
name:  include:    - "testing-.*" # include only resource names that have `testing-` prefix  exclude: [] To match all resources except those that have `testing-` prefix, use the following constraint: name: include: - ".*" # include all resource names... exclude: - "testing-.*" # ...except those that have `testing-` prefix ### Event types[​](#event-types"DirectlinktoEventtypes")
...
event:  message:    include:      - "^Back-off.*" # match all events with message starting with `Back-off`    exclude: [] To match all events except those with message starting with `Back-off`, use the following constraint: event: message: include: - ".*" # match all event messages... exclude: - "^Back-off.*" # ...except those starting with `Back-off` ### Recommendations[​](#recommendations"DirectlinktoRecommendations")
