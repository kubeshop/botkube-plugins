Title: Getting Started with the New Botkube Slack App
...
 name: Botkube  
 description: Botkube  
 background\_color: "#a653a6"  
features:  
 bot\_user:  
   display\_name: Botkube  
   always\_online: false  
oauth\_config:  
 scopes:  
   bot:  
     - channels:read  
...
 token\_rotation\_enabled: false
...
\`export SLACK\_API\_BOT\_TOKEN="xoxb-{botToken}"\`

The Slack app-level token from earlier:

\`export SLACK\_API\_APP\_TOKEN="xapp-{appToken}"\`
...
\--set settings.clusterName=${CLUSTER\_NAME} \\

\--set executors.kubectl-read-only.kubectl.enabled=${ALLOW\_KUBECTL} \\
